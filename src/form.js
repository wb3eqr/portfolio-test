export function initContactForm(containerId, t) {
  ...
  container.innerHTML = `
    <form id="contact-form" class="space-y-4" action="https://formspree.io/f/xqapwqry" method="POST">
      ...
      <textarea name="message" rows="3" placeholder="${t.contact.msgPlaceholder}" required class="w-full px-4 py-3 rounded-xl bg-[var(--bg-secondary)] border border-[var(--glass-border)] text-[var(--text)] text-sm outline-none transition-all duration-300 focus:border-[var(--accent)] focus:shadow-[0_0_0_3px_var(--accent-glow)] resize-none" style="caret-color:var(--accent)"></textarea>
      <button type="submit" class="btn-primary w-full px-5 py-3 rounded-xl font-medium text-sm flex items-center justify-center gap-2 group">
        <svg class="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
        ${t.contact.send}
      </button>
    </form>`;
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      form.innerHTML = `<div class="text-center py-10 fade-enter"><svg class="w-14 h-14 mx-auto mb-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><p class="text-xl font-semibold">${t.contact.thanks}</p><p class="text-sm mt-2" style="color:var(--text-secondary)">${t.contact.reply}</p></div>`;
